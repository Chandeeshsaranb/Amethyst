...
export default function ProductDetails() {
  ...
  const product = useMemo(() => {
    ...
    return storedProducts.find((item) => String(item.id) === String(id)) || null;
  }, [id, location.state]);
  ...
  const container = {
    ...
    margin: "0 auto",
    display: "grid",
    gridTemplateColumns: "95px 1.1fr 1fr",
    gap: "22px",
    alignItems: "start",
  };
      ...
      <div style={container}>
        ...
        <div style={details}>
          ...
          {product.categories?.Occasions?.length > 0 && (
            <div style={detailLine}>
              <strong>Occasions:</strong> {product.categories.Occasions.join(", ")}
            </div>
          )}
